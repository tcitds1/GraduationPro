...
class UserSpider():
    ...
    def analyse_movie_page(self, response, start, url, marks):
        ...
        for user in user_list:
            self.db.changeTable('user_backup')
            if(self.db.exists(user_page)):
                self.db.changeTable('wait_user')
    ...
    def analyse_user_page(self, user_data):
        ...
        while True:
            ...
            self.change_proxy()
        if(response=='next'):
            self.db.changeTable('unvisited_user')
            # self.db.update_one(url, {'user_page':url})
            self.db.put({'user_page',url})
            self.log.info('UserSpider : {}---用户{}爬取失败，存入[unvisited_user]数据表----------'.format(time.ctime(),url.replace('https://www.douban.com/people/','')))
            return
        ...
        try:
            ...
            remove_number = '^' + re.sub('\D', '', user_data['user_name'])
